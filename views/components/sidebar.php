    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col bg-[#011C27] text-white transition-transform duration-300 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0">
        ...
        <nav class="flex-1 space-y-2 px-4 py-6">
            <?php foreach ($items as $item): ?>
                ...
                <a href="<?= e(url_for($item['href'])) ?>" class="flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-semibold transition <?= $isActive ? 'bg-[#EB9FEF] text-[#011C27] shadow-lg shadow-[#EB9FEF]/20' : 'text-white/78 hover:bg-white/10 hover:text-white' ?>">
